public class Level {
    ...
    public boolean edge(int x, int y, int dist, int dir){
        boolean k = false;
            switch (dir) {
                case 0:
                    if(y >= dist){
    ...
    public boolean[] neighbor(int x, int y, int testT){
        ...
        if(edge(x, y, 1, 0) && t[x][y-1] == testT){
            k[0] = true;
        }
        if(edge(x, y, 1, 1) && t[x][y+1] == testT){
            k[1] = true;
        }
        if(edge(x, y, 1, 2) && t[x-1][y] == testT){
            k[2] = true;
        }
        if(edge(x, y, 1, 3) && t[x+1][y] == testT){
            k[3] = true;
        }
        return k;
    }
    public boolean Oneighbor(int x, int y, int testT){
        boolean k = false;
        if(edge(x, y, 1, 0) && o[x][y-1] == testT){
            k = true;
        }
        if(edge(x, y, 1, 1) && o[x][y+1] == testT){
            k = true;
        }
        if(edge(x, y, 1, 2) && o[x-1][y] == testT){
            k = true;
        }
        if(edge(x, y, 1, 3) && o[x+1][y] == testT){
            k = true;
        }
        if(turn == 1){
            k = true;
    ...
    public boolean surround(int x, int y, int own, boolean zero){
        ...
        if(edge(x, y, 1, 0) && edge(x, y, 1, 2)){
            if((zero && o[x-1][y-1] == 0) || o[x-1][y-1] == own){
                k++;
            }
        }
        if(edge(x, y, 1, 0) && edge(x, y, 1, 3)){
            if((zero && o[x+1][y-1] == 0) || o[x+1][y-1] == own){
                k++;
            }
        }
        if(edge(x, y, 1, 1) && edge(x, y, 1, 2)){
            if((zero && o[x-1][y+1] == 0) || o[x-1][y+1] == own){
                k++;
            }
        }
        if(edge(x, y, 1, 1) && edge(x, y, 1, 3)){
            if((zero && o[x+1][y+1] == 0) || o[x+1][y+1] == own){
                k++;
            }
        }
