...
public class Game{
    ...
    public Game(Level gen){
        this.gen = gen;
        unit = new ArrayList<>();
        unit.add(new Unit(gen, 0, 0, 10, 20));
    }
    public void city(int x, int y, int own){
        if(own==1&&gen.cityA[1]==0){
            while(gen.t[x][y]==0){
    ...
    public void makeUnit(int x, int y, int pl){
        if(gen.u[x][y]==0){
            if(gen.gold[pl]>=50 && gen.o[x][y]==pl && gen.popu[pl]>=3){
                unit.add(new Unit(gen, x, y, pl, 20));
                gen.gold[pl]-=50;
                gen.popu[pl]-=3;
            }
        }else{
    ...
    public void combat(Unit a, Unit d){
        int ap = 10, dp = 5;
        if(gen.t[a.x][a.y]==3||gen.t[a.x][a.y]==0)ap+=5;
        if(gen.t[d.x][d.y]==3||gen.t[d.x][d.y]==0)dp+=5;
        if(dp==ap){a.alive=false;d.alive=false;}
        else if(dp>ap)a.alive=false;
        else{d.alive=false;a.x=d.x;a.y=d.y;}
    }
    public Unit GeU(int i){
        return ((Unit) unit.get(i));
    }
    public boolean Uneighbor(int x, int y, int own){
        if(gen.edge(x, y, 1, 0) && gen.u[x][y-1] != 0 && GeU(gen.u[x][y-1]).pl == own){
            return true;
        }
        if(gen.edge(x, y, 1, 1) && gen.u[x][y+1] != 0 && GeU(gen.u[x][y+1]).pl == own){
            return true;
        }
        if(gen.edge(x, y, 1, 2) && gen.u[x-1][y] != 0 && GeU(gen.u[x-1][y]).pl == own){
            return true;
        }
        if(gen.edge(x, y, 1, 3) && gen.u[x+1][y] != 0 && GeU(gen.u[x+1][y]).pl == own){
            return true;
        }
        if(gen.edge(x, y, 1, 2) && gen.edge(x, y, 1, 0) && gen.u[x-1][y-1] != 0 && GeU(gen.u[x-1][y-1]).pl == own){
            return true;
        }
        if(gen.edge(x, y, 1, 3) && gen.edge(x, y, 1, 1) && gen.u[x+1][y+1] != 0 && GeU(gen.u[x+1][y+1]).pl == own){
            return true;
        }
        if(gen.edge(x, y, 1, 1) && gen.edge(x, y, 1, 2) && gen.u[x-1][y+1] != 0 && GeU(gen.u[x-1][y+1]).pl == own){
            return true;
        }
        return gen.edge(x, y, 1, 0) && gen.edge(x, y, 1, 3) && gen.u[x+1][y-1] != 0 && GeU(gen.u[x+1][y-1]).pl == own;
    }
    public void AI(int pl, boolean first){
        if(first){
            ...
            while(gen.gold[pl] >= 10&&!done(pl, 0)){
            ...
            while(gen.gold[pl] >= 10 && !done(pl, 1)){
                for(int cx = 0; cx < gen.width; cx++){
                    for(int cy = 0; cy < gen.height; cy++){
                        upgrade(cx, cy, pl);
                    }
            ...
            for(int x = 0; x < gen.width; x++){
                for(int y = 0; y < gen.height; y++){
                    if(gen.o[x][y] == pl){
                        System.out.println(gen.popu[pl]);
                        makeUnit(x, y, pl);
                    }
                }
            }
    ...
    public boolean done(int pl, int func){
        for(int cx = 0; cx < gen.width; cx++){
            for(int cy = 0; cy < gen.height; cy++){
                if(func == 0 && gen.o[cx][cy] == pl && gen.i[cx][cy] == 0 && gen.t[cx][cy] != 0){
                    return false;
                }
                if(func == 1 && gen.o[cx][cy] == pl && (gen.i[cx][cy] == 1 || gen.i[cx][cy] == 3) && gen.t[cx][cy] != 0){
                    return false;
                }
            }
        }
    ...
    public void reveal(int x, int y){
        if(gen.surround(x, y, 1, false)||Uneighbor(x, y, 1)||GeU(gen.u[x][y]).pl == 1)
            gen.r[x][y] = true;
    }
    public void update(){
        //turn();
